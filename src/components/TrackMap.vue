...
const updateCarPosition = () => {
  if (!circuitPath.value) return;
  
  const pathLength = circuitPath.value.getTotalLength();
  
  const distance = (100 - props.progress) * pathLength / 100;
  
  const point = circuitPath.value.getPointAtLength(distance);
  carPosition.value = { x: point.x, y: point.y };
};
